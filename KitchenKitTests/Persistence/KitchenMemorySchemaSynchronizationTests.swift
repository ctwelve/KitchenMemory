...
final class KitchenMemorySchemaSynchronizationTests: XCTestCase {
  ...
  func testV4StoreMigratesToV5WithRecipeAndSessionSnapshotIntact() throws {
    let directory = FileManager.default.temporaryDirectory
      .appending(path: "KitchenMemoryV4ToV5Tests")
    ...
    let recipeID = UUID()
    let revisionID = UUID()
    let snapshot = Data("retained V4 snapshot".utf8)
    ...
    do {
      ...
      context.insert(KitchenRecord(id: kitchenID, name: "V4 Kitchen"))
      context.insert(RecipeRecord(
        id: recipeID,
        kitchenID: kitchenID,
        currentRevisionID: revisionID
      ))
      context.insert(RecipeRevisionRecord(
        id: revisionID,
        recipeID: recipeID,
        revisionNumber: 1,
        title: "V4 Recipe",
      ...
      context.insert(CookingSessionRecord(
        ...
        snapshotDigest: Data(repeating: 1, count: 32),
        sourceSessionID: nil,
        sourceClosureID: nil
      ))
      try context.save()
    ...
    do {
      migrated = try ModelContainer(
        for: schema,
        migrationPlan: KitchenMemoryMigrationPlan.self,
        configurations: [configuration]
      )
    } catch {
      XCTFail("V5 migration failed: \(error)")
      return
    }
    let context = ModelContext(migrated)

    XCTAssertEqual(migrated.schema.version, Schema.Version(5, 0, 0))
    XCTAssertEqual(try context.fetch(FetchDescriptor<RecipeRecord>()).first?.id, recipeID)
    XCTAssertEqual(
      try context.fetch(FetchDescriptor<RecipeRevisionRecord>()).first?.title,
      "V4 Recipe"
    )
    XCTAssertEqual(
      try context.fetch(FetchDescriptor<CookingSessionRecord>()).first?.snapshotData,
      snapshot
    )
    XCTAssertTrue(try context.fetch(FetchDescriptor<RecipeSaveRecord>()).isEmpty)
    XCTAssertTrue(try context.fetch(FetchDescriptor<RecipeSelectionRecord>()).isEmpty)
    XCTAssertTrue(try context.fetch(FetchDescriptor<RecipePruneRecord>()).isEmpty)
  }
