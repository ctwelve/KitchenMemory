...
struct RecipeEditorView: View {
  ...
  private var source: RecipeSource? {
    guard text(sourceTitle) != nil || text(sourceAuthor) != nil || text(sourcePublisher) != nil || URL(string: sourceURL) != nil else { return nil }
    return RecipeSource(kind: sourceKind, title: text(sourceTitle), authorName: text(sourceAuthor), publisherName: text(sourcePublisher), canonicalURL: URL(string: sourceURL))
  }
  ...
  var body: some View {
    EditorDisclosureGroup(
      ...
      ForEach(section.ingredients.indices, id: \.self) { index in
        IngredientEditor(ingredient: $section.ingredients[index], moveUp: { move(index, by: -1) }, moveDown: { move(index, by: 1) }, delete: { section.ingredients.remove(at: index) })
      }
      ...
      HStack { Button("Move Up", action: moveUp); Button("Move Down", action: moveDown); Spacer(); Button("Delete Section", role: .destructive, action: delete) }
    }
  }
  private var titleBinding: Binding<String> { Binding(get: { section.title ?? "" }, set: { section.title = $0 }) }
  private func move(_ index: Int, by offset: Int) { let destination = index + offset; guard section.ingredients.indices.contains(destination) else { return }; section.ingredients.swapAt(index, destination) }
}
...
private struct IngredientEditor: View {
  ...
  var body: some View {
    EditorDisclosureGroup(
      ...
      HStack { Button("Move Up", action: moveUp); Button("Move Down", action: moveDown); Spacer(); Button("Delete", role: .destructive, action: delete) }
    }
  }
  private func optionalBinding(_ keyPath: WritableKeyPath<RecipeIngredient, String?>) -> Binding<String> { Binding(get: { ingredient[keyPath: keyPath] ?? "" }, set: { ingredient[keyPath: keyPath] = $0 }) }
}
...
private struct InstructionSectionEditor: View {
  ...
  var body: some View {
    EditorDisclosureGroup(
      ...
      ForEach(section.steps.indices, id: \.self) { index in
        InstructionEditor(step: $section.steps[index], moveUp: { move(index, by: -1) }, moveDown: { move(index, by: 1) }, delete: { section.steps.remove(at: index) })
      }
  ...
  private var titleBinding: Binding<String> { Binding(get: { section.title ?? "" }, set: { section.title = $0 }) }
  private func move(_ index: Int, by offset: Int) { let destination = index + offset; guard section.steps.indices.contains(destination) else { return }; section.steps.swapAt(index, destination) }
}
...
private struct InstructionEditor: View {
  ...
  private var nameBinding: Binding<String> { Binding(get: { step.name ?? "" }, set: { step.name = $0 }) }
}
