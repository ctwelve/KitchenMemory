...
if [[ ! -d "$framework" || ! -f "$product_app/Contents/embedded.provisionprofile" ]]; then
  echo "The signed Kitchen Memory Develop products were not produced." >&2
  exit 1
fi

mkdir -p "$app/Contents/MacOS" "$app/Contents/Frameworks"
cp "$tool_directory/Info.plist" "$app/Contents/Info.plist"
cp "$product_app/Contents/embedded.provisionprofile" \
...
codesign --force --sign "$identity" --timestamp=none \
  --entitlements "$tool_directory/SessionCloudKitAcceptance.entitlements" \
  "$app"
