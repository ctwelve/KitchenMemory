...
class CITest < Minitest::Test
  ...
  def test_daily_build_checks_production_without_archiving
    ...
    assert_equal %w[analyze production], jobs.map(&:first)
  ...
  def test_aggregate_rejects_skipped_failed_and_cancelled_jobs
    ...
    assert_equal %w[repository build mac-tests], job.fetch('needs')
    script = job.fetch('steps').first.fetch('run')
    good = {'REPOSITORY_RESULT' => 'success', 'BUILD_RESULT' => 'success', 'MAC_TEST_RESULT' => 'success'}
    assert Open3.capture3(good, 'bash', '-c', script).last.success?
    good.keys.product(%w[failure skipped cancelled]).each do |key, result|
